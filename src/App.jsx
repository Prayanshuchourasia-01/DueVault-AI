...
import { BellRing, Bell, Eye, X, ShieldAlert } from 'lucide-react';
...
function App() {
  ...
  const { sendNotification, permission: notifPermission, askPermission } = useNotifications();
  const [permBannerDismissed, setPermBannerDismissed] = useState(() => localStorage.getItem('duevault_perm_banner_dismissed') === 'true');
  ...
  // Removed mobile redirect so Dashboard is accessible on phones
  ...
  return (
    <div className={`h-[100dvh] w-full overflow-hidden bg-slate-950 text-slate-200 font-sans selection:bg-cyan-500/30 flex flex-col md:flex-row print:bg-white print:text-black ${theme === 'light' ? 'light-theme' : ''}`}>
        ...
        {/* Main Content Area */}
        <main className="flex-1 h-full overflow-y-auto overflow-x-hidden px-4 pt-[calc(4rem+env(safe-area-inset-top))] md:p-8 pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-8 print:p-0 print:h-auto print:overflow-visible">
          ...
          {/* Notification Permission Banner (one-time, user gesture) */}
          {!permBannerDismissed && notifPermission !== 'granted' && (
            <div className="bg-indigo-900/40 border border-indigo-500/30 rounded-xl p-4 flex items-center justify-between gap-4 mb-6 animate-fade-in print:hidden">
              <div className="flex items-center gap-3 min-w-0">
                <div className="p-2 bg-indigo-500/20 rounded-lg text-indigo-400 flex-shrink-0">
                  <Bell className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <h4 className="font-bold text-slate-200 text-sm">Enable Notifications</h4>
                  <p className="text-xs text-slate-400">Get alerts when timetable blocks start/end and Pomodoro breaks begin.</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={askPermission} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-all cursor-pointer">Enable</button>
                <button onClick={() => { setPermBannerDismissed(true); localStorage.setItem('duevault_perm_banner_dismissed', 'true'); }} className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-all cursor-pointer"><X className="w-4 h-4" /></button>
              </div>
            </div>
          )}
